...
from model import Stable_PROTO
# import os
...
def train_ones(p0, class_name2, support2, support_label, query2, model, val_test_task_step, task_lr, N, K):
...
def train_one_batch(args, class_name, support0, support_label, query0, query_label, model, train_task_step, task_lr, it):

    N = args.N
    K = args.K
    class_name1, support1, query1 = model.coder(class_name), model.coder(support0), model.coder(query0)  # [N/N*K/N*L, max_length, 768]
    class_name2, support2, query2 = model(class_name1), model(support1), model(query1)  # [N/N*K/N*L, 256]
    p0 = model.mlp(class_name2)  # [256, 256]
    if torch.isnan(p0).int().sum() != 0:
        print("P0 is nan.")
    logits_q = train_ones(p0, class_name2, support2, support_label, query2, model, train_task_step, task_lr, N, K)
...
def train_model(model, args):
    ...
    for it in range(train_iter):
        ...
        loss_q, right_q = train_one_batch(args, class_name, support, support_label, query, query_label, model, train_task_step, task_lr, it)
